...
rev_list=[]
# # csv 파일의 1행(카페이름) 읽기
file_dir = 'C:/Users/손병우/Desktop/read_data/place_read.csv'
with open(file_dir,'r', encoding='CP949') as csvfile:
  ...
  for row in reader:
    ...
    rev_list.append(rev_csv)
i=1
star_list=[]
review_list=[]
fail_list=[]

ff = open(r'C:/Users/손병우/Desktop/regex_data/place_new.csv', 'w', encoding='CP949', newline='')
csvWriter = csv.writer(ff)
type=input('카테고리는? : ')          # 카테고리
for place,loc,rev in zip(place_name_list,location_list,rev_list):
  try:
    search = loc + ' ' + place            
    page1(search)
    ...
    try:
      csvWriter.writerow([place, loc,type,star,rev,review_img])
    except:

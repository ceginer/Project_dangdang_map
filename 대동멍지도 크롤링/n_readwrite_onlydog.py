...
review_list=[]
# csv 파일의 1행(카페이름) 읽기
file_dir = 'C:/Users/손병우/Desktop/Daedong_Mung!/onlydog.csv'
# encoding='utf-8'
with open(file_dir,'r', encoding='CP949') as csvfile:
  ...
  for row in reader:
    ...
    cafe_name_list.append(cafe_name_csv)
    location_list.append(location_csv)


# (선희) csvWriter는 여기서 열어야 해! for문 밖에서
ff = open(r'C:/Users/손병우/Desktop/Daedong_Mung!/onlydog_edit.csv', 'a', encoding='CP949', newline='')
...
i =499
# 각 카페장소에 맞는 내용 크롤링
for place in cafe_name_list[499:]:

  print(i)

  place = location_list[i] + ' ' + place
  i+=1
  page1(place)
  ...
  try:
    for type in dict_prepare['category']:
      if "애견카페" in type:
        type = '애견전용'
  except:
    continue
  ...
  try:
    menuInfo=dict_prepare['menuInfo'] # 메뉴
  except:
    menuInfo=''
  ...
  try:
    if type=='애견전용':
      csvWriter.writerow([name, location,address,phone,type,menuInfo,hourInfo,link1,desc,main_img, mapx, mapy])
  except:
    if type=='애견전용':

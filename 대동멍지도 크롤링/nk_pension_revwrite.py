...
loc_list=[]
# csv 파일의 1행(카페이름) 읽기
file_dir = 'C:/Users/손병우/Desktop/Daedong_Mung!/pension_edit.csv'
with open(file_dir,'r', encoding='CP949') as csvfile:
  reader = csv.reader(csvfile)
  for row in reader:
    if row[0]:
      pension_name_csv=row[0]
    if row[1]:
      loc_csv=row[1]
    pension_name_list.append(pension_name_csv)
    loc_list.append(loc_csv)
    # location_list.append(location_csv)
    # review_list.append(review_csv)

print(pension_name_list)

# (선희) csvWriter는 여기서 열어야 해! for문 밖에서
ff = open(r'C:/Users/손병우/Desktop/pension_finnnn.csv', 'w', encoding='CP949', newline='')
...
i =0
# 각 카페장소에 맞는 내용 크롤링
for place,loc in zip(pension_name_list,loc_list):
  place = loc + place
  page1(place)
...
# 카카오 api 에서 얻어온 x,y 를 y,x로 바꾸어서 넣어주었음.
  if ('펜션' or '강아지' or '반려견' or '형견' or '애견' or '반려동물') in desc:
    try:
      csvWriter.writerow([name, location,address,phone,'펜션',link1,desc,main_img, mapy, mapx,booking_url])
    except:
      csvWriter.writerow([name, location,address,phone,'펜션',link1,'',main_img, mapy, mapx, booking_url])
